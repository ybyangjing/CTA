...
def life_experience(model, continuum, x_te, args):
    ...
    #img_stored = model.img_stored

    time_end = time.time()
    time_spent = time_end - time_start

    # return torch.Tensor(result_t), torch.Tensor(result_a), img_stored, time_spent
...
if __name__ == "__main__":
    ...
    parser.add_argument('--theta', default=0, type=float,
                        help='threshold for GradCAM mastk.)')
    ...
    # result_t, result_a, img_stored, spent_time = life_experience(
    ...
    # print the final number of images saved in episodic memory for each task
    #print(img_stored)
